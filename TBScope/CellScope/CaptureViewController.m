...
@synthesize managedObjectContext,currentUser,currentSlide,snapButton,analyzeButton,bleConnectionLabel,holdTimer;
...
{
    [[TBScopeContext sharedContext] setMicroscopeLED:CSLEDBrightfield Level:0];
    [[TBScopeContext sharedContext] setMicroscopeLED:CSLEDFluorescent Level:0];
...
- (IBAction)didTouchUpStageButton:(id)sender
{
    ...
    if (buttonPressed.tag==7) //BF
    {
        ...
        {
            [[TBScopeContext sharedContext] setMicroscopeLED:CSLEDBrightfield Level:70];
    ...
    else if (buttonPressed.tag==8) //FL
    {
        ...
        {
            [[TBScopeContext sharedContext] setMicroscopeLED:CSLEDFluorescent Level:255];
    ...
    else //stage control button
    {
        [holdTimer invalidate];
        holdTimer = nil;
        [[TBScopeContext sharedContext] disableMotors];
...
-(void) moveStage:(NSTimer *)timer
{
    if (self.currentSpeed==CSStageSpeedSlow)
        [[TBScopeContext sharedContext] moveStageWithDirection:self.currentDirection Steps:20 DisableAfter:NO];
    else if (self.currentSpeed==CSStageSpeedFast)
        [[TBScopeContext sharedContext] moveStageWithDirection:self.currentDirection Steps:100 DisableAfter:NO];
}
    ...
    while (state!=0) {
        ...
        switch (state) {
            case 1: //reset
                ...
                break;
            case 2: //backup
                [[TBScopeContext sharedContext] moveStageWithDirection:CSStageDirectionFocusUp
                ...
                break;
            case 3:
                [[TBScopeContext sharedContext] moveStageWithDirection:CSStageDirectionFocusDown
...
- (void) autoscan
{
    ...
    for (int i=0;i<30;i++)
    {
        [[TBScopeContext sharedContext] moveStageWithDirection:CSStageDirectionDown
                                                         Steps:100
                                                  DisableAfter:NO];
        [NSThread sleepForTimeInterval:0.1];
    }
    ...
    for (int i=0;i<60;i++)
    {
        [[TBScopeContext sharedContext] moveStageWithDirection:CSStageDirectionUp
                                                         Steps:100
                                                  DisableAfter:NO];
        [NSThread sleepForTimeInterval:0.1];
    }
