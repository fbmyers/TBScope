...
- (void)uploadSlide;
{
    ...
    NSString* fileName = [NSString stringWithFormat:@"%@ - %@-%d-%d",
                                        ...
                                        self.slideToUpload.patientID,
                                        self.slideToUpload.slideNumber,
                                        self.slideToUpload.readNumber];
    ...
    NSArray* arrayToSerialize = [NSArray arrayWithObjects:self.slideToUpload,nil];
    ...
    [self.driveService executeQuery:query
                  completionHandler:^(GTLServiceTicket *ticket,
                      ...
                      if (error == nil)
                      {
                          ...
                          NSDictionary* info = [[NSDictionary alloc] initWithObjectsAndKeys:@0,@"imageNumber",nil];
...
- (void)uploadImage:(NSNotification*)notification
{
    ...
    NSLog(@"uploading image %d",imageNum);
    
    if (imageNum>=self.slideToUpload.slideImages.count)
    {
        NSLog(@"all images uploaded");
        //TODO: mark this slide as completely uploaded
        dispatch_async(dispatch_get_main_queue(),
                       ^{
        [self.waitIndicator dismissWithClickedButtonIndex:0 animated:YES];
                       });
    ...
    //TODO: orderedSet, make sure this works w/ field number
    //TODO: same handler for assetlib and bundle
    Images* currentImage = (Images*)[[[self.slideToUpload slideImages] allObjects] objectAtIndex:imageNum];
        ...
        [library assetForURL:aURL resultBlock:^(ALAsset *asset)
         {
             ...
             else
             {
                 ...
                 NSString* fileName = [NSString stringWithFormat:@"%@ - %@-%d-%d - %d",
                                       [[NSUserDefaults standardUserDefaults] stringForKey:@"CellScopeID"],
                                       self.slideToUpload.patientID,
                                       self.slideToUpload.slideNumber,
                                       self.slideToUpload.readNumber,
                 ...
                 [self.driveService executeQuery:query
                               completionHandler:^(GTLServiceTicket *ticket,
                                   ...
                                   if (error == nil)
                                   {
                                       ...
                                       NSDictionary* info = [[NSDictionary alloc] initWithObjectsAndKeys:[NSNumber numberWithInt:imageNum+1],@"imageNumber",nil];
                                       [[NSNotificationCenter defaultCenter] postNotificationName:@"UploadImage" object:self.slideToUpload userInfo:info];
                                   }
                                   else
                                   {
                                       NSLog(@"An error occurred: %@", error);
                       ...
                       ^{
                           ...
                           [self.driveService executeQuery:query
                                         completionHandler:^(GTLServiceTicket *ticket,
                                             ...
                                             {
                                                 NSLog(@"File ID: %@", insertedFile.identifier);
                                                 NSNumber *nextNumber = [NSNumber numberWithInt:(imageNum+1)];
                                                 NSDictionary* info = [[NSDictionary alloc] initWithObjectsAndKeys:nextNumber,@"imageNumber",nil];
                                                 [[NSNotificationCenter defaultCenter] postNotificationName:@"UploadImage" object:self.slideToUpload userInfo:info];
                                             }
                                             else
                                             {
